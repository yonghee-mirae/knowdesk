...
⚠️ **DB 축소 수단 추가 (2026-08-23):** 문서가 삭제/제외돼도 SQLite는 자동으로 파일 크기를 줄이지 않는다(FTS5는 `DELETE`만으로 세그먼트 저장공간을 회수하지 않고, 빈 페이지도 `VACUUM` 없이는 반환되지 않는다). `PRAGMA incremental_vacuum`는 이 환경에서 실측 결과 호출당 겨우 1페이지만 회수해 사실상 동작하지 않는 것으로 확인돼 채택하지 않았다. 대신 `Db::reclaim_space()`(`core/src/db/mod.rs`)가 `content_fts` `optimize` → 일반 `VACUUM` → `PRAGMA wal_checkpoint(TRUNCATE)` 순으로 실행해 실제 파일 크기를 회수한다(WAL 모드라 VACUUM 결과가 `-wal` 사이드카에만 남고 메인 파일에 반영 안 되는 문제가 있어 체크포인트가 필수였다). 문서 삭제/폴더 제외/전체 리셋 등 정리가 발생하는 모든 경로에서 이 함수를 호출한다 — 상세는 `03_Architecture.md` Index Queue 항목 참조.
