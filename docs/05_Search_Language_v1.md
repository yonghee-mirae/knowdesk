...
`AND`/`OR`/`NOT`은 대소문자를 구분하지 않는다(`and`/`or`/`not`, `And`도 동일하게 연산자로 인식됨) — 2026-08-22 수정. 그전에는 소문자로 쓰면 연산자가 아니라 그 글자 그대로를 찾는 검색어로 취급돼, 예를 들어 `채권 or 발행`이 조용히 검색 결과 0건이 되는 버그가 있었다.
