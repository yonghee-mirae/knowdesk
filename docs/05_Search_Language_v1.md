...
⚠️ **ASCII 영숫자 구간은 예외 — 버그 수정 (2026-08-23):** 위 슬라이딩 윈도우는 한국어(및 구두점 섞인 문자열)에만 적용되고, 단어 안의 **순수 ASCII 영숫자 구간(run)은 통째로 하나의 토큰으로 유지**된다. 원래는 한국어의 "어간+조사가 띄어쓰기 없이 붙는" 문제를 보완하려는 장치인데, 영어에는 그런 현상이 없어서 그대로 적용하면 오히려 부작용만 생겼다 — 실사용 중 "IAM"(AWS 용어)이 "IA"/"AM"으로 쪼개져 전혀 무관한 `am` 검색에 걸리는 문제, 그리고 문장 끝에 구두점이 붙은 "diagram?"도 "ra"/"am"/"m?" 등으로 쪼개져 같은 문제를 일으키는 걸 확인했다. 지금은 단어를 "ASCII 영숫자 구간 vs. 그 외" 런(run)으로 먼저 나눈 뒤, ASCII 구간만 통째로 토큰화하고 나머지(한국어·구두점)는 기존 방식대로 슬라이딩 윈도우를 적용한다(`bigrams_of_word`). 부수 효과로 "TEST하는"처럼 영문 단어가 한글 조사에 바로 붙은 경우도 이제 Kiwi 없이 "TEST" 토큰만 깔끔하게 뽑힌다.

⚠️ **최소 검색어 길이 게이트 (2026-08-23):** 위와 별개로, 프론트엔드(`frontend/src/main.ts`)는 일반 검색어(필터/연산자/따옴표 구문/접두 검색 제외)가 2자 미만이면 디바운스 후에도 검색 자체를 발화하지 않는다 — 짧고 흔한 검색어(영문 관사 "a" 등)가 만드는 저효율/광범위한 매치 부하를 원천 차단하기 위함. 상세는 `12_UI_Spec.md` C1 "동작" 참조.
