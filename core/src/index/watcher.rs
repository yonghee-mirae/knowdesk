...
///
/// `ModifyKind::Any` is included alongside `Data`/`Name` because Windows' backend
/// (`ReadDirectoryChangesW`) reports `FILE_ACTION_MODIFIED` as `ModifyKind::Any` -
/// it never emits `Data`/`Metadata`/`Name` the way Linux (inotify) and macOS
/// (fsevent) do. Without this, an ordinary file save on Windows (e.g. hand-editing
/// `settings.json`, or editing a file inside a watched folder) was silently ignored -
/// confirmed on real Windows hardware (2026-08-26): `watched_folders` additions
/// never took effect until the app was restarted. Safe on Linux/macOS - neither
/// backend ever produces `ModifyKind::Any`, so this is a Windows-only behavior change.
