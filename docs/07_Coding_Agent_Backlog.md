...
⚠️ **kdfind 전용 병렬 색인 추가 (2026-08-25):** GUI가 지켜야 하는 유휴 CPU 목표(TASK-306 참조)가 kdfind에는 적용되지 않는다는 점에 착안해, `cli/src/parallel_index.rs`(kdfind 전용, `core`/`src-tauri` 무변경)로 워커 스레드 여러 개가 해싱·추출·bigram 토크나이즈를 병렬 수행하도록 확장했다. SQLite 쓰기는 `Mutex<Connection>`으로 직렬화(`rusqlite::Connection`이 `Send`이지만 `Sync`는 아님), Kiwi는 `!Send`라 전용 액터 스레드(`KiwiHandle`, `src-tauri`의 `KiwiActor`와 동일 패턴을 `cli` 안에 독립 재구현)로 처리.

⚠️ **실사용 크래시 발견·수정 — libpdfium.so 동시 호출 (2026-08-25):** 위 병렬 색인을 실제 홈 디렉터리(`kdfind ~/ 채권`)에 돌렸다가 SIGSEGV로 죽는 걸 실사용 중 발견 — 코어덤프(`/var/crash`, gdb로 확인)에서 두 스레드가 동시에 libpdfium.so 내부(`CPDF_ColorSpace::CreateBufAndSetDefaultColor` / `FPDF_LoadPage`)를 실행 중이었다. 설계 검토 때 "PDFium 자체는 스레드 세이프하지 않지만 `pdfium-render`가 내부적으로 뮤텍스로 직렬화해준다"고 그 크레이트 README만 보고 판단해 우리 쪽 락을 안 넣었던 게 원인 — 실제로는 크레이트 소스 어디에도 그런 뮤텍스가 없다(`thread_safe` 피처는 `unsafe impl Send + Sync`만 붙임, README와 실제 코드가 다름). `parallel_index.rs`에 `PDF_LOCK`(`Mutex<()>`)을 추가해 PDF 파일 처리에서만 직렬화하도록 수정. 재현 테스트(동일 PDF 40개, 16스레드) 확인 — 락 없이 5/5 크래시, 있으면 반복 실행에도 안정적. 상세는 `docs/13_CLI_Tool.md` 참조.

3000개 파일 코퍼스 실측(수정 후 재측정): 순차 1분 31초 → 병렬(16코어) 27초.
